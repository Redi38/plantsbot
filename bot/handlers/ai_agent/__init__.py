...
переименовать группу, показать список, а также всё, что умеет меню
«💧 Полив»: создать зону, отметить полив, отложить, сменить интервал/время/
название, удалить, показать зоны).

Пакет разбит по сценариям, а не по типу кода — вся логика одного сценария
целиком лежит в одном файле:
  states.py         — общие FSM-состояния (AIAdd, AIDelete, AIEdit, AIZone)
  ...
  zone_flow.py       — зоны полива: создание, «полил», отложить, интервал,
                       время напоминания, переименование, удаление, показ
